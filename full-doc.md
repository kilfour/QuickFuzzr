...
var folderCfg =
    from files in Configr<FolderEntry>.Property(a => a.Files,
        Fuzzr.One<FileEntry>().Many(1, 2).ToList())
    from folders in Configr<FolderEntry>.Property(a => a.Folders,
        Fuzzr.One<FolderEntry>().Many(1, 2).ToList())
    select Intent.Fixed;
```
**Combined Fuzzr**  
```csharp
...
- Works seamlessly in LINQ chains and with .Select(...) to offset or transform the sequence.  
...
| [Configr.Combine](#configrcombine)| Combines multiple configuration operations into one. |
#### Configr&lt;T&gt;.Ignore
...
| Method| Description |
| -| - |
| [Apply](#apply)| Executes a side-effect for each value generated by the wrapped fuzzr. |
...
| [Mutate](#mutate)| Chooses a mutation strategy and applies it a specified number of times. |
...
| [ToArray](#toarray)| Creates a Fuzzr that materializes the sequence produced by the source Fuzzr as an `Array<T>`. |
| [ToList](#tolist)| Creates a Fuzzr that materializes the sequence produced by the source Fuzzr as a `List<T>`. |
