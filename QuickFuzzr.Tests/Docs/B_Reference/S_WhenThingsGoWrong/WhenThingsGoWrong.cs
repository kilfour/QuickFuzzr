...
namespace QuickFuzzr.Tests.Docs.B_Reference.S_WhenThingsGoWrong;

[DocFile]
[DocFileHeader("When Things Go Wrong")]
[DocContent(Content)]
...
Exception messages include possible solutions tailored to the failure.";
    ...
    [Fact]
    public void DocumentsEveryQuickFuzzrException()
    {
        var exceptionTypes = typeof(QuickFuzzrException).Assembly
            .GetTypes()
            .Where(type =>
                type != typeof(QuickFuzzrException) &&
                !type.IsAbstract &&
                typeof(QuickFuzzrException).IsAssignableFrom(type))
            .ToArray();

        Assert.NotEmpty(exceptionTypes);
        Assert.All(exceptionTypes,
            type => Assert.Contains($"`{type.Name}`", Content));
    }
}
