...
public class E_Counter
{
    ...
    public void Counter_Instances_Are_Independent()
    {
        var gen =
            ...
            from b in Fuzzr.Counter("b").Select(x => x + 11)
            select (a, b);
    ...
    [Fact]
    [DocContent("- Works seamlessly in LINQ chains and with .Select(...) to offset or transform the sequence.")]
    public void Counter_Apply_Preserves_Sequence()
        => Assert.Equal([12, 13, 14], Fuzzr.Counter("a").Select(x => x + 11).Many(3).Generate());
