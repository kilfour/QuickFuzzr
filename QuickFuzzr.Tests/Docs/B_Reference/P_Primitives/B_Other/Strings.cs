...
public class Strings
{
    ...
    [Fact]
    [DocContent("- The default string generator can be replaced for the current generation run with `Configr.Primitive(...)`.")]
    public void ReplaceDefaultGenerator()
    {
        var fuzzr =
            from _ in Configr.Primitive(Fuzzr.String(Fuzzr.Char('-', '-'), 3, 3))
            from str in Fuzzr.String()
            select str;
