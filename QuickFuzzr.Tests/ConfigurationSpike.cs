...
public class ConfigurationSpike
{
    ...
    {
        var cfg =
            from _1 in Config<Order>.Property(a => a.OrderId, Counter())
            from _2 in Config.Property(a => a.Name == "Quantity", Fuzzr.Int(100, 200))
            from _3 in Config<Order>.Property(a => a.Item, Fuzzr.OneOf("toy", "phone", "apple"))
            select Unit.Instance;
        var ordersGen =
            from _ in cfg
            from order1 in Fuzzr.One<Order>()
            from __ in Config<Order>.Property(a => a.Item, "OVERRIDE")
